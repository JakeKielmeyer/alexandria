...
//
// Dropdown panel anchored to the canvas toolbar — replaces the old
// center-screen modal. Single entry point for all media in the editor.
//
// Trigger button "Assets Folder" lives in the canvas toolbar (rendered by
// this same component so the dropdown can position relative to it).
//
// Upload section:
//   • Multiple files at once — progress shows "N / M".
//   • On success: registers in `assets`, creates a layer on the active
//     panel (if one is active), asset appears in grid.
//
// Assets grid:
...
export default function AssetsFolder(): React.JSX.Element {
  ...
  const handleUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    const panel = panelId
      ? useEditorStore.getState().panels.find((p) => p.id === panelId) ?? null
      : null
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const path = panelId && panel
          ? panelLayerPath(story.id, panelId, file)
          : `${story.id}/assets/${Date.now()}_${file.name}`
        ...
        if (panelId && panel) {
          const panelLayers = useEditorStore.getState().layers.filter((l) => l.panel_id === panelId)
          const newLayer = {
            panel_id: panelId,
            story_id: story.id,
          ...
          const { data, error: layerError } = await supabase
            ...
            .select()
            .single()
          if (layerError || !data) throw layerError
  ...
  }, [story, activePanelId, setAssets, addLayer, setSaveStatus, pushToast])
      ...
      {assetsModalOpen && (
        <div className="assets-folder-dropdown" role="dialog" aria-label="Story assets">
          ...
          <div className="asset-upload-strip">
            ...
            />
            {!activePanelId && (
              <span className="asset-upload-hint">Select a panel to add uploads to it</span>
            )}
          </div>
